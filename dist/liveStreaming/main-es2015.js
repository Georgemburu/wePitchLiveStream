...
let BroadcastPageComponent = class BroadcastPageComponent {
    constructor() {
        this.page = "";
    }
    ngOnInit() {
...
let ChatService = class ChatService {
    ...
    streamLive(cb) {
        this.wsService.streamLive((data) => {
            cb(data);
        });
    }
    sendStreamPackets(data) {
        this.wsService.sendStreamPackets(data);
    }
    receiveStreamPackets(cb) {
        this.wsService.receiveStreamPackets((data) => {
            cb(data);
        });
    }
    fetchVideo(url, cb) {
...
let PlaybackPageComponent = class PlaybackPageComponent {
    ...
    ngOnInit() {
        ...
        this.chatService.playbackVideoFiles((files) => {
            console.log(files);
            this.FILES = JSON.parse(files);
        });
        this.modal = document.getElementById('video_modal');
    }
    handleVideoFileClick(e) {
        console.log(e);
        let fileText = e.target.innerText;
        this.video_title = fileText;
        // open modal
        this.OPEN_MODAL = true;
        this.modal.classList.remove('show_not');
        console.log(e.target.classList);
        let playback_video_elem = document.getElementById('playback_video_elem');
...
PlaybackPageComponent.ctorParameters = () => [
...
let VideoComponent = class VideoComponent {
    ...
    ngOnInit() {
        ...
        if (document.URL.toLowerCase().includes('broadcaster')) {
            ...
            let chunks = [];
            let FullvideoData = [];
            this.FULLVIDEO = FullvideoData;
            if (navigator.mediaDevices) {
                navigator.mediaDevices.getUserMedia({ audio: true, video: { height: 500, width: 500 } }).then((stream) => {
                    ...
                    recorder.ondataavailable = (ev) => {
                        if (ev.data.size > 0) {
                            ...
                            this.chatService.sendVideo(ev.data);
                            console.log(stream);
                            FullvideoData.push(ev.data);
            ...
            this.page = 'viewer';
            let video = document.querySelector('video');
            let main_video_div = document.querySelector('.main_video_div');
            // video.remove()
            ...
            // let blobArrays = []
            this.chatService.receiveStreamPackets((data) => {
...
let VideocameraService = class VideocameraService {
    constructor() { }
    connect() {
    ...
    receiveStreamPackets(cb) {
        this.socket.on('receiveStreamPackets', (e) => {
    ...
    playbackVideoFiles(cb) {
        this.socket.on('playbackVideoFiles', (e) => {
            cb(e);
        });
    }
};
